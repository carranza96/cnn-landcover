...
from Pavia import Pavia_CV_Postprocessing,Pavia_CV_Decoder
import os
import pandas as pd
import CNNTrain_2D
...
for discarded_indices, selected_indices in dataset_reduction.split(X, y):
    ...
    for cv_index in range(5):
        ...
        for train_index, test_index in skfold.split(X_reduced, y_reduced):
            ...
            raw, train_acc, test_acc = Pavia_CV_Decoder.decode(input, config, train_positions, test_positions, save_path)
            ...
            if apply_filter:
                filt_img, post_test_acc = Pavia_CV_Postprocessing.apply_modal_filter(input, raw, train_positions, test_positions)
            else:
                filt_img, post_test_acc = Pavia_CV_Postprocessing.clean_image(input, raw), test_acc


            conf_matrix = Pavia_CV_Postprocessing.get_conf_matrix(input, filt_img, test_positions)
